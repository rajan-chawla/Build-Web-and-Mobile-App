...
userRoutes.get("/api/get/profile", function(req, res) {
  ...
  const query =
    ...
    "(select count(*) from `hsfuldadb`.`user_role` where user_id = id and role_id = 3) as isAdmin,CAST(is_seller_requested AS SIGNED INTEGER) AS is_seller_requested  from `hsfuldadb`.`user` where id='" +
    id +
    "'";
